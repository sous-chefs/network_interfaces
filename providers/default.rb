action :save do
  ...
  template "/etc/network/interfaces.d/#{new_resource.device}" do
    ...
    variables(
      ...
      bond_master:  Chef::Recipe::NetworkInterfaces.value(:bond_master,  new_resource.device, new_resource, node),
      bond_primary: Chef::Recipe::NetworkInterfaces.value(:bond_primary, new_resource.device, new_resource, node),
